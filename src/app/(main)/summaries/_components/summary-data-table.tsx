...
import { AddNewSummaryForm } from "./add-new-summary-form";

export function SummartyDataTable({
  courses,
}: {
  ...
  return (
    <div className="p-2 h-full flex flex-col gap-3">
      <div className="flex items-center justify-between">
        <h3 className="text-3xl font-bold">Summary Collections</h3>
        <AddNewSummaryForm isExceedLimit={courses.length >= 3} />
      </div>
      <div className="bg-secondary drop-shadow-sm flex-1 rounded-xl p-4 overflow-y-auto custom-scrollbar space-y-2">
        {courses.length === 0 ? (
          <div className="h-full w-full flex justify-center items-center font-semibold text-muted-foreground">
            You have no summaries available, create a new one!
          </div>
        ) : (
          courses.map((course) => (
